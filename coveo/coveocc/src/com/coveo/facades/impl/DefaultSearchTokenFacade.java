...
public class DefaultSearchTokenFacade implements SearchTokenFacade {
    private static final Logger LOG = Logger.getLogger(com.coveo.facades.impl.DefaultSearchTokenFacade.class);
    ...
    public SearchTokenBody buildRequestBody(String userId, Set<String> userGroupIds) {
        ...
        userGroupIds.forEach(userGroupId -> userTokenIds.add(createSearchTokenUserId(userGroupId, "Group")));
    ...
    public SearchTokenUserId createSearchTokenUserId(String name, String type) {
    ...
    @Override
    public ResponseEntity<SearchTokenWsDTO> getSearchToken(String baseSiteId, String userId, long maxAgeMilliseconds, String userAgent) {
        BaseSiteModel baseSite = getBaseSiteService().getBaseSiteForUID(baseSiteId);
        ...
        if (LOG.isTraceEnabled()) {
            ...
            LOG.trace("URI: " + uri);
            LOG.trace("User ids: [ " + searchTokenBody.getUserIds().stream().map(userInfo -> SensitiveDataMaskingUtils.maskEmail(userInfo.getName())).collect(Collectors.joining(", ")) + " ]");
    ...
    /**
     * Method used to get all the user price groups for a given user. These values can be used to build the search token request and provide authorization in the JWT for user specific pricing
     * @param userId
     * @return list of user price group ids
     */
    public Set<String> getUserPriceGroups(String userId) {
        Set<String> userPriceGroupIds = new HashSet<>();
        UserModel user = getUserService().getUserForUID(userId);
    ...
    public HttpHeaders buildHeaders(String coveoApiKey, String userAgent) {
    ...
    public RestTemplate getSearchTokenRestTemplate() {
        return searchTokenRestTemplate;
    }

    @Required
    public void setSearchTokenRestTemplate(RestTemplate searchTokenRestTemplate) {
        this.searchTokenRestTemplate = searchTokenRestTemplate;
    }
