...
public class UpdateStreamService {
    ...
    public void init(StreamEnabledSource source, String[] userAgents) {
        init(source, userAgents, false);
    }

    /**
     * Initializes the UpdateStreamService with the given source and user agents.
     *
     * @param source The stream-enabled source (e.g., CatalogSource) to push updates to.
     * @param userAgents The user agent strings to identify the client.
     * @param useCatalogQueue If true, uses CatalogDocumentUploadQueue to support shallow merge operations.
     *                        Set to true when working with Catalog sources that require addOrMerge functionality.
     */
    public void init(StreamEnabledSource source, String[] userAgents, boolean useCatalogQueue) {

        BackoffOptions backoffOptions = new BackoffOptionsBuilder().withMaxRetries(configurationService.getConfiguration().getInt("coveopushclient.maxretries", DEFAULT_MAX_RETRIES))
                .withRetryAfter(configurationService.getConfiguration().getInt("coveopushclient.retryafter.milliseconds", DEFAULT_RETRY_AFTER)).build();
    ...
    /**
     * Adds documents to an open file container be created or updated. If there is no file container open to receive the documents, this function will open a file container before uploading documents into it.
     *
     ...
     * @throws InterruptedException If the creation of the file container or adding the document is interrupted.
    ...
    /**
     * Adds a document containing the specific field, and it's value to be updated. If there is no file container open to receive the documents, this function will open a file container before uploading the partial update details into it. More
     * details on partial updates can be found in the <a href="https://docs.coveo.com/en/l62e0540/coveo-for-commerce/how-to-update-your-catalog#partial-item-updates"> Partial item updates</a> section.
     *
     * <p>If called several times, the service will automatically batch documents and create new
     * stream chunks whenever the data payload exceeds the <a href="https://docs.coveo.com/en/lb4a0344#stream-api-limits">batch size limit</a> set for the Stream API.
     *
     ...
     * <pre>{@code
     * //...
     * UpdateStreamService service = new UpdateStreamService(catalogSource));
     * for (ShallowMergeDocument document : documentsToMerge) {
     *     service.addShallowMerge(document);
     * }
     * service.close();
     * }</pre>
     *
     * @param document The shallow merge document to push to your file container
     * @throws InterruptedException If the creation of the file container or adding the document is interrupted.
     * @throws IOException          If the creation of the file container or adding the document fails.
     * @throws UnsupportedOperationException If the service was not initialized with a Catalog source queue.
     ...
     */
    public void addShallowMerge(ShallowMergeDocument document) throws IOException, InterruptedException {
        try {
            updateStreamServiceInternal.addShallowMerge(document);
        } catch (Exception e) {
            LOG.error("Error adding shallow merge document", e);
            throw e;
        }
    }

    /**
     * Adds documents to an open file container be deleted. If there is no file container open to receive the documents, this function will open a file container before uploading documents into it.
     *
     * <p>If called several times, the service will automatically batch documents and create new
     * stream chunks whenever the data payload exceeds the <a href="https://docs.coveo.com/en/lb4a0344#stream-api-limits">batch size limit</a> set for the Stream API.
     *
    ...
    /**
     * Sends any buffered documents and <a href="https://docs.coveo.com/en/l62e0540/how-to-update-your-catalog#step-3-send-the-file-container-to-update-your-catalog">pushes the file container</a>.
     *
     ...
     */
    public void close() throws IOException, InterruptedException, NoOpenFileContainerException {
        try {
